...
struct llama_layer {
    ...
    struct ggml_tensor * w2_b; // ffn_down bias // 
    struct ggml_tensor * w3_b; // ffn_up bias // 
            ...
            case LLM_ARCH_GPTNEOX:
                {
                    ...
                    for (uint32_t i = 0; i < n_layer; ++i) {
                        ...
                        if (backend == GGML_BACKEND_GPU) {
                            vram_weights +=
                                ggml_nbytes(layer.attn_norm) + ggml_nbytes(layer.attn_norm_b) +
                                ggml_nbytes(layer.wqkv)      + ggml_nbytes(layer.wqkv_b)      + 
...
static struct ggml_cgraph * llama_build_graph(
    ...
    switch (model.arch) {
        ...
        case LLM_ARCH_GPTNEOX:
            {
                result = llm_build_gptneox(lctx, tokens, embd, n_tokens, n_past);
            } break;
        case LLM_ARCH_GPTJ:
            {
                result = llm_build_gptj(lctx, tokens, embd, n_tokens, n_past);
            } break;
        case LLM_ARCH_GPT2:
            {
                result = llm_build_gpt2(lctx, tokens, embd, n_tokens, n_past);
            } break;
        default:
            GGML_ASSERT(false);
    };
